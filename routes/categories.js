...
const { CategoryModel, validteCategory } = require("../models/categoryModel");
...
router.get("/category/:url_code", async (req, res) => {
    ...
    try {
        let data = await CategoryModel.find({ cat_code: url_code })
            .limit(perPage)
            .skip((page - 1) * perPage)
            .sort({ [sort]: reverse })
        res.status(200).json(data);
...
router.get("/search", async (req, res) => {
    ...
    try {
        let data = await CategoryModel.find({ $or: [{ name: regQuery }, { info: regQuery }] })
            .limit(20)
            .skip(0)
        res.status(200).json(data);
...
router.post("/", auth, async (req, res) => {
    ...
    let validateBady = validteCategory(req.body);
